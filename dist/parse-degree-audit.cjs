...
// src/parse-degree-audit-cli.ts
var STATUS_FILTERS = ["all", "unfulfilled", "in_progress", "complete"];
function main() {
  ...
  const statusFilter = statusFilterArg ?? "all";
  ...
  let html3;
  try {
    html3 = (0, import_node_fs.readFileSync)(filePath, "utf8");
  } catch (err) {
    console.error(
  ...
  try {
    ...
    const requirements = statusFilter === "all" ? audit.requirements : audit.requirements.filter((r) => r.status === statusFilter);
    console.log(
      JSON.stringify(
        {
